...
func LihatTopikTerurut() {
	...
	var tagCount [NMAX][2]string
	var tagCountIndex int

	// Count tags
	for i := 0; i < JumlahKonsultasi; i++ {
		found := false
		for j := 0; j < tagCountIndex; j++ {
			if tagCount[j][0] == Konsultasi[i].Tags {
				count := 0
				for k := 0; k < len(tagCount[j][1]); k++ {
					count = count*10 + int(tagCount[j][1][k]-'0')
				}
				count++
				tagCount[j][1] = fmt.Sprintf("%d", count)
				found = true
				break
			}
		}
		if !found {
			tagCount[tagCountIndex][0] = Konsultasi[i].Tags
			tagCount[tagCountIndex][1] = "1"
			tagCountIndex++
		}
	}
	...
	// Selection Sort
	for i := 0; i < tagCountIndex-1; i++ {
		maxIndex := i
		for j := i + 1; j < tagCountIndex; j++ {
			count1 := 0
			count2 := 0
			for k := 0; k < len(tagCount[maxIndex][1]); k++ {
				count1 = count1*10 + int(tagCount[maxIndex][1][k]-'0')
			}
			for k := 0; k < len(tagCount[j][1]); k++ {
				count2 = count2*10 + int(tagCount[j][1][k]-'0')
			}
			if count1 < count2 {
				maxIndex = j
			}
		}
		tagCount[i], tagCount[maxIndex] = tagCount[maxIndex], tagCount[i]
	}

	for i := 0; i < tagCountIndex; i++ {
		fmt.Printf("Tag: %s ; Jumlah Pertanyaan: %s\n", tagCount[i][0], tagCount[i][1])
	}
}
